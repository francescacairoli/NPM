...
def active_label_query(number_points, scaler, local_guide, rej_rule, n_emp_samples, decision_threshold=None):
	start = time.time()
	...
	samples_avg_pred_classes, samples_avg_probs, samples_std_probs = predictions_on_set(x_samples_scaled_t, number_points, n_emp_samples, local_guide)
...
def average_predictions(x, n_samples, local_guide):
    sampled_models = [local_guide(None, None) for _ in range(n_samples)]
    yhats = [modello(x).data for modello in sampled_models]
    mean = torch.mean(torch.stack(yhats), 0).numpy()
    std = torch.std(torch.stack(yhats), 0).numpy()
    if bern:
    	pred_class = np.round(mean)
    else:
    	pred_class = np.argmax(mean)
    #pred_class = np.round(mean)
    if bern:
    	return pred_class, mean, std
    else:
    	return pred_class, mean[pred_class], std[pred_class]
    #return pred_class, mean, std


def predictions_on_set(x_t, n_points, n_samples, local_guide):
    pred_classes = np.zeros(n_points)
    pred_avgs = np.zeros(n_points)
    pred_stds = np.zeros(n_points)
    for j in range(n_points):
        pred_class, pred_prob, pred_dev = average_predictions(x_t[j,:], n_samples, local_guide)
        pred_classes[j] = pred_class.item()
        pred_avgs[j] = pred_prob.item()
        pred_stds[j] = pred_dev.item()
    return pred_classes, pred_avgs, pred_stds
